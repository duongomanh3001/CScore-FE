...
export class AssignmentService {
   ...
   */
  static async getTeacherAssignments(): Promise<any[]> {
    return apiClient.get<any[]>('/api/teacher/assignments');
  }

  /**
   * Get assignments by teacher ID (for dashboard)
   */
  static async getAssignmentsByTeacher(teacherId: number): Promise<any[]> {
    return this.getTeacherAssignments();
  }

  /**
   * Get assignments with pagination for teacher
   */
  static async getTeacherAssignmentsPaginated(page = 0, size = 10, sortBy = 'createdAt', sortDir = 'desc') {
    return apiClient.get<any>(`/api/teacher/assignments/paginated?page=${page}&size=${size}&sortBy=${sortBy}&sortDir=${sortDir}`);
  }

  /**
   * Get assignments by course for teacher
   */
  static async getTeacherAssignmentsByCourse(courseId: number): Promise<any[]> {
    return apiClient.get<any[]>(`/api/teacher/courses/${courseId}/assignments`);
  }

  /**
   * Get assignment details for teacher
   */
  static async getTeacherAssignmentById(assignmentId: number): Promise<DetailedAssignmentResponse> {
    return apiClient.get<DetailedAssignmentResponse>(`/api/teacher/assignments/${assignmentId}`);
  }

  /**
   * Create assignment (Teacher only)
   */
  static async createAssignment(assignmentData: CreateAssignmentRequest): Promise<DetailedAssignmentResponse> {
   ...
   */
  static async updateAssignment(assignmentId: number, assignmentData: any): Promise<DetailedAssignmentResponse> {
    return apiClient.put<DetailedAssignmentResponse>(`/api/teacher/assignments/${assignmentId}`, assignmentData);
  }

  /**
   * Delete assignment (Teacher only)
   */
  static async deleteAssignment(assignmentId: number): Promise<MessageResponse> {
    return apiClient.delete<MessageResponse>(`/api/teacher/assignments/${assignmentId}`);
  }

  /**
   * Toggle assignment status (Teacher only)
   */
  static async toggleAssignmentStatus(assignmentId: number): Promise<DetailedAssignmentResponse> {
    return apiClient.patch<DetailedAssignmentResponse>(`/api/teacher/assignments/${assignmentId}/toggle-status`);
  }
}
