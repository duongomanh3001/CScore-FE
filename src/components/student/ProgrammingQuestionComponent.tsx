...
export default function ProgrammingQuestionComponent({
  ...
  const handleRunCode = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        setCodeOutput(result.output || 'Code chạy thành công (không có output)');
      } else {
      ...
      {question.exampleTestCases && question.exampleTestCases.length > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-semibold text-green-900">Ví dụ Test Cases:</h3>
            <button
      ...
      <div className="bg-white border border-slate-300 rounded-lg">
        <div className="flex items-center justify-between p-3 border-b border-slate-200 bg-slate-50">
          ...
          <div className="flex gap-2">
            ...
            >
              ▶️ {isChecking ? 'Đang chạy...' : 'Chạy thử'}
            </button>
            <button
      ...
      {testResults && (
        <div className={`border rounded-lg p-4 ${
          ...
          <div className="mb-3">
            <span className={`font-medium ${
              ...
              {testResults.success 
                ...
                : `⚠️ Cần cải thiện! ${testResults.passedTests || 0}/${testResults.totalTests || 0} test cases đạt yêu cầu`}
            </span>
          </div>
          ...
          <div className="mt-3 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs text-yellow-800">
            💡 <strong>Lưu ý:</strong> Đây là kết quả kiểm tra tạm thời. Điểm cuối cùng được xác định khi bạn nộp bài và được chấm với đầy đủ test cases từ giảng viên.
          </div>
        </div>
      )}
    </div>
